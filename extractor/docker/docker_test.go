...
func TestDockerExtractor_SaveLocalImage(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			requiredFiles: []string{"helloworld.txt", "badworld.txt"},
		},
		{
			name:          "happy path with cache hit",
			cacheHit:      true,
			requiredFiles: []string{"foofromcache.txt"},
		},
		// TODO: Add a sad path test for bad nested tar layers
	}
	...
	for _, tc := range testCases {
		...
		if tc.cacheHit {
			...
			dst := e.EncodeAll([]byte("foofromcache.txt"), nil)
		...
		for _, rf := range tc.requiredFiles {
			assert.Contains(t, string(actualSavedTarBytes[:]), rf, tc.name) // TODO: Strengthen assertion
		}
		...
		for _, rf := range tc.requiredFiles {
			assert.Contains(t, string(actualStoredValue), rf, tc.name)
		}
	}
}
